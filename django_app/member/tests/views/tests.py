...
class ChangeProfileImageViewTest(TestCase):
    def test_user_not_authenticated(self):
        url_change_profile_image = '/member/profile/change/'
    ...
    def test_uses_chage_profile_image_template(self):
        make_user_and_login(self.client)
        response = self.client.get('/member/profile/change/')
        self.assertTemplateUsed(response, 'member/change-profile.html')

    def test_url_resolves_to_change_profile_image_view(self):
        make_user_and_login(self.client)
        found = resolve('/member/profile/change/')
